...
def test_get_failed_reminders(admin_client):
    client, headers = admin_client
    organization_id = 1  # Replace with actual organization ID from setup
    response = client.get(f"{settings.API_V1_STR}/admin/customers/{organization_id}/failed-reminders", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert "failed_reminders" in data
    assert isinstance(data["failed_reminders"], list)